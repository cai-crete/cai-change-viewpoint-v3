...
## 9. N09 프로토콜 구성 — 실제 구현 예시

N09(Change Viewpoint)는 **5개의 독립적 Principle Protocol**로 구성됩니다.
하나의 통합 프로토콜이 아닌, `selectedView` 입력 값에 따라 해당 프로토콜이 독립 트리거됩니다.

### 9.1 프로토콜 트리거 맵

| Protocol 파일 | 트리거 조건 | 렌즈 | 투시 유형 |
|-------------|-----------|------|---------|
| [n09-protocol-bird-eye-view.md](../references/n09-protocol-bird-eye-view.md) | `selectedView === 'birdEye'` | 24mm | 2점 투시 |
| [n09-protocol-eye-level-view.md](../references/n09-protocol-eye-level-view.md) | `selectedView === 'eyeLevel'` | 24mm TS | 2점 투시 |
| [n09-protocol-front-view.md](../references/n09-protocol-front-view.md) | `selectedView === 'front'` | 50mm TS | 1점 투시 |
| [n09-protocol-side-view.md](../references/n09-protocol-side-view.md) | `selectedView === 'rightSide'` | 50mm TS | 1점 투시 |
| [n09-protocol-top-view.md](../references/n09-protocol-top-view.md) | `selectedView === 'top'` | 24mm | 직교 투영 |

### 9.2 N09 Protocol의 구조적 패턴 — Photographer's Workflow

N09 Protocol은 §4 표준 5-섹션 구조를 따르되, ACTION PROTOCOL 단계에서
**Photographer's Workflow** 패턴을 채택합니다.
건축 사진의 구체적 촬영 기법이 AI의 시점 렌더링 행동 지침으로 직접 변환되는 방식입니다.

| 표준 섹션 | N09 구현 방식 |
|---------|-------------|
| GOAL | 건축물 시점 변환 목표 (기하·재료 불변 원칙 포함) |
| CONTEXT | 입력 이미지를 "불변의 물리적 좌표값"으로 선언 (Ontological Locking) |
| ROLE | 건축 사진가 / Architectural Simulation Engine & Virtual Photographer |
| ACTION PROTOCOL | Photographer's Workflow: Positioning → Photographic Focus → Lighting → Lens & Perspective Control |
| Specification | 카메라·렌즈·조리개·ISO·셔터 스피드 (Prompt JSON 포함) |

### 9.3 불변 상수 (N09 공통)

모든 N09 Protocol이 공유하는 불변 상수 — 어떤 뷰 타입에서도 변경 불가:

| 상수 | 설명 |
|------|------|
| Geometry | 건물의 높이·폭·비율 |
| Materials | 외장 재료·마감·텍스처 |
| Proportion | 구조적 요소 간 비례 |

변경 가능한 것: **Angle of View (시점)** + **Optical Environment (광학 환경)**만 허용.
